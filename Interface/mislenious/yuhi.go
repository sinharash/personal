...
>>>>>>
same thing but more debugger in execute because commandUsed not seeing
...
// Updated execute function with debugging for commandUsed
func execute(cmd *cobra.Command, flags *Flags) {
	if !terraform.IsLockFileSupported() {
		core.ErrorMsg("Terraform lock file generation requires Terraform 0.14+ or OpenTofu")
		return
	...
	// DEBUG: Let's see what's in os.Args when we get to this point
	core.StdMsg("DEBUG - os.Args length: " + fmt.Sprintf("%d", len(os.Args)))
	core.StdMsg("DEBUG - Full os.Args: " + strings.Join(os.Args, " | "))
	
	// Get the command name from os.Args for display
	commandUsed := "unknown"
	if len(os.Args) >= 2 {
		commandUsed = os.Args[1]
		core.StdMsg("DEBUG - os.Args[1]: '" + commandUsed + "'")
	} else {
		core.StdMsg("DEBUG - os.Args length is less than 2!")
	}
	
	core.StdMsg("DEBUG - Final commandUsed: '" + commandUsed + "'")
	core.StdMsg("Using " + tool + " (detected from command " + commandUsed + ")")
